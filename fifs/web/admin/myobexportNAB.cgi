...
#
# $Header: svn://svn/SWM/trunk/web/admin/myobexportNAB.cgi 10075 2013-12-01 22:59:02Z tcourt $
#
...
sub getOptions {
    ...
    my $body = qq[
        <form action="$target" method="post">
            ...
            <input type="radio" name="paymentType" id="rbPaypal" value="PAYPAL_NAB" class="ROnb" checked><label for="rbPaypal"> NAB</label>
...
sub buildDoc {
    ...
    #$paymentType = ($paymentType eq 'PAYPAL_NAB') # maybe make this a constant or even a Defs entry
    #    ? $Defs::GATEWAY_PAYPAL
    #    : $Defs::GATEWAY_CC;
    ...
    $wherePP   .= ($paymentType eq 'PAYPAL_NAB') ? qq[=] : qq[<>];
    $wherePP .= qq['PAYPAL_NAB'];

	my $sql = qq[
      SELECT 
				IF(l.intClubID>0,
					IF((cPay.strShortLegalName <> '' and cPay.strShortLegalName IS NOT NULL), cPay.strShortLegalName,
						IF((cPay.strOrgName <> '' and cPay.strOrgName IS NOT NULL), cPay.strOrgName, c.strName)
					)                   
					, IF((aPay.strShortLegalName <> '' and aPay.strShortLegalName IS NOT NULL) , aPay.strShortLegalName,
						IF((aPay.strOrgName <> '' and aPay.strOrgName IS NOT NULL), aPay.strOrgName, a.strName))
				) AS strName,
      	IF(l.intClubID>0, cPay.strStreetAddress1, aPay.strStreetAddress1) AS strAddress1, 
      	IF(l.intClubID>0, cPay.strStreetAddress2, aPay.strStreetAddress2) AS strAddress2, 
      	...
      	IF(l.intClubID>0, cPay.strParentCode, aPay.strParentCode) AS strParentCode, 
				l.intClubID,
				l.intAssocID,
				l.intLogType,
				l.strCurrencyCode,
      ...
      FROM 
        ...
        WHERE
					TL.intPaymentType IN ($Defs::PAYMENT_ONLINENAB)
...
sub getMyobExportID {
    ...
    my $gatewayNo = ($paymentType eq 'PAYPAL_NAB' or $paymentType == 2)
        ? $Defs::GATEWAY_NAB
        : $Defs::GATEWAY_CC;
...
sub doExport {
    ...
    my $subject  = "AUD Data Export - Payment Split Fees";
    my $retval   = emailExportData($dbh, '', $emailAddress, $message, '', $subject, $filename, $doc, '');
    my $body     = '';
    
    if ($retval == 0) {
        $body = qq[<p class="OKmsg">The Payment Split Export File for AUD MYOB has been emailed to $emailAddress.</p><br>];
...

        {
            ...
            my $subject  = "AUD Data Export - Customer List";
	...
	if (1==2) {
            ...
            my $message  = "The customer list for export ID $myobExportID is included in the attached file ($filename)" ;
            my $subject  = "AUD Data Export - PayPal Fees";
