...
#
# $Header: svn://svn/SWM/trunk/web/admin/supportyoursport_clubs.cgi 10208 2013-12-10 00:34:50Z tcourt $
#
...
sub validate_params {
    ...
    if ($campaign_id) {
        $campaign_id = $Defs::CAMPAIGN_REBEL if $campaign_id == $Defs::CAMPAIGN_SYS_REBEL;
        $campaign_id = $Defs::CAMPAIGN_AMART if $campaign_id == $Defs::CAMPAIGN_SYS_AMART;
    }
...
sub show_results {
    ...
    my $where = '';
    $where .= " AND (DATE_FORMAT(cc.dtRegistered, '%Y-%m-%d')>='$date_from2')" if $date_from2;
    $where .= " AND (DATE_FORMAT(cc.dtRegistered, '%Y-%m-%d')<='$date_to2')" if $date_to2;

    my $st = qq[
        SELECT COUNT(DISTINCT cc.intClubID) as COUNTNUM 
        FROM tblClubCampaigns cc
        WHERE (cc.intCampaignID=$campaign_id) $where
    ];
    ...
    $st = qq[
        SELECT
            DISTINCT cc.intClubID,
            strRealmName, 
            a.strName as strAssocName, 
            c.strName as strClubName,
            DATE_FORMAT(cc.dtRegistered, '%d/%m/%Y') as dtRegistered
        FROM tblClubCampaigns cc
            INNER JOIN tblAssoc a ON a.intAssocID=cc.intAssocID 
            INNER JOIN tblClub c ON c.intClubID=cc.intClubID 
            INNER JOIN tblRealms r ON r.intRealmID=a.intRealmID 
        WHERE (cc.intCampaignID=$campaign_id) $where
        ORDER BY strRealmName, strAssocName, strClubName
     ];
