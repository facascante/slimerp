...
<table cellpadding=8 cellspacing=0 border=1>
	...
	while (my $dref =$query->fetchrow_hashref())	{
			 ...
			 if(!defined $dref->{$key})      {
				...
				$clubName[$dref->{'C2clubID'}] = $dref->{'2013C'} if($dref->{'C2clubID'} and (!$clubName[$dref->{'C2clubID'}]));	
				$assocName[$dref->{'A1assocID'}] = $dref->{'2012A'} if($dref->{'A1assocID'} and (!$assocName[$dref->{'A1assocID'}] ));	
				$assocName[$dref->{'A2assocID'}] = $dref->{'2013A'} if($dref->{'A2assocID'} and (!$assocName[$dref->{'A2assocID'}]));	
	...
	foreach my $j (keys %{$clubCount{$i}})        {
		my $line1 = $assocName[$i] || '';
		my $clubtoA = $clubToAssoc[$j] || 0;
		my $line2 = $assocName[$clubtoA] || '';
		my $line3 = $clubName[$j] || '';
		my $origTotal = keys %{$origCount{$i}} || 0;
		my $assocTotal = keys %{$assocCount{$i}{$clubtoA}} || 0;
		my $clubTotal = keys %{$clubCount{$i}{$j}} || 0;
		print STDERR qq[$origTotal|$line1|$assocTotal|$line2|$clubTotal|$line3\n];

	  }
	 }

#	use Data::Dumper;
#	print STDERR Dumper(@assocName);
