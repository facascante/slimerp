#
# $Header: svn://svn/SWM/trunk/web/admin/CompAdmin.pm 11309 2014-04-15 22:28:35Z ppascoe $
#
...
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw( handleManageLadderTeams display_comp_list );
@EXPORT_OK = qw( handleManageLadderTeams display_comp_list );
...
sub display_comp_list {
  my ($db, $action, $intAssocID, $intClubID, $target) = @_;
  ...
  $query->execute($assocID_IN) or query_error($statement);
  my $count = 0;
  my $body = '';
  while (my $dref = $query->fetchrow_hashref()) {
    ...
    my $extralink='';
    if($dref->{intRecStatus} < 1) {
      $classborder.=" greytext";
      $extralink=qq[ class="greytext"];
    }
    my $hash_value = AdminCommon::create_hash(0,0,$intAssocID,0,0);
    my $edit_link = qq[<a href="$target?action=ASSOC_comps_manage&intCompID=$dref->{intCompID}&intAssocID=$intAssocID&hash=$hash_value">[Edit]</a>];
    my $status = '';
    $status = 'Active' if ($dref->{'intRecStatus'} == 1);
    $status = 'Inactive' if ($dref->{'intRecStatus'} == 0);
    $status = 'Deleted' if ($dref->{'intRecStatus'} == -1);
    $body.=qq[
      <tr>
        <td class="$classborder">$dref->{intCompID}</td>
        <td class="$classborder">$dref->{strCompName}</td>
        <td class="$classborder">$status</td>
        <td class="$classborder">$dref->{strSeasonName}</td>
  ...
  if (!$body) {
    ...
    $body=qq[
     <table cellpadding="1" cellspacing="0" border="0" width="95%" align="center">
      <tr>
        <th style="text-align:left;">Comp ID</th>
        <th style="text-align:left;">Comp Name</th>
        <th style="text-align:left;">Status</th>
        <th style="text-align:left;">Season Name</th>
...
sub handleManageLadderTeams {
  ...
  my $resultHTML = '';
  my $title = '';
  my $compID = param('intCompID') || 0;
  my $assocID = param('intAssocID') || 0;
  my $action = param('type') || '';
  ...
  $Data->{'client'} = qq[action=ASSOC_comps_manage&intAssocID=$assocID&intCompID=$compID&hash=$hash_value];
	...
	elsif ($action =~ /MLT_clear_team_club/) {
    _clear_team_club($Data);
	}
...
sub _display_menu {
  ...
  <div style="background-color:white;padding:10px;">
		<p style="color:red;font-size:16px;font-weight:bold;">The following page should be used with caution. If you do not understand the consequences of using the options on this page then leave this page NOW !</p>
		<br>
    <h1>Teams in a Competition</h1>
		<p><a href="index.cgi?$Data->{'client'}&type=MLT_reset_comp_teams" onclick="return confirm('Are you sure you wish to regenerate Competition Team Statuses ?')">Regenerate Competition Teams Status</a>: This will regenerate the below list to only show teams that are fixtured in a match for this competition.</p>
		<br>
		$comp_teams_status
		<br> 
		<br> 
    <h1>Team Movement</h1>
		<p><a href="index.cgi?$Data->{'client'}&type=MLT_reset_regradelog" onclick="return confirm('Are you sure you reset the timestamps ?')">Reset Add/Remove Timestamps</a>: This will reset the timestamps for the below table back to earliest date in the fixture. Any team that does not appear in the fixture will also be removed from the below table. If you have regraded teams in and/or out of the competition then do not use this option.</p>
		<br>
    ...
    <h1>Fixtures</h1>
		...
		<br>
		<br>
		<p><a href="index.cgi?$Data->{'client'}&type=MLT_reset_match_num" onclick="return confirm('Are you sure you wish to reset the match numbers in the Fixture ?')">Regenerate Match Numbers for a Fixture</a>: This will reset the match numbers for a competition to remove any duplicates that may be causing matches or rounds not to display.</p>
		<br>
		<br>
		<p>If the above tools do not resolve your issues, please contact the SportingPulse Technical Team and quote Competition ID $Data->{'clientValues'}{'compID'} and the ID of the problem team(s).</p>
  </div>
	];
	return $resultHTML;
}
...
sub _display_regrade_log {
  ...
  foreach my $key (keys %{$teams}) {
    next if $key eq 'earliest_fixture_date';
		next if ($key == -1);
    $teams_ddl .= qq[<option value="$key">$teams->{$key}{'Name'}</option>];
  }
	my $st = qq[
		SELECT
			tblRegradeLog.*,
	...
	while (my $href = $q->fetchrow_hashref()) {
		$regrade_log_html .= qq[
			<tr>
				...
				<td>[<a href="index.cgi?$Data->{'client'}&type=MLT_rl_delete&id=$href->{'intRegradeLogID'}">Delete</a>]</td>
			</tr>
		];
	}
	$regrade_log_html = qq[<tr><td colspan="4">No records found</td>] unless($regrade_log_html);
    my $hash_value = AdminCommon::create_hash(0,0,$Data->{'clientValues'}{'assocID'},0,0);
	$regrade_log_html = qq[
		<table class="listTable">
			...
			<form>
				...
				<input type="hidden" name="action" value="ASSOC_comps_manage">
				<input type="hidden" name="intAssocID" value="$Data->{'clientValues'}{'assocID'}">
				<input type="hidden" name="intCompID" value="$Data->{'clientValues'}{'compID'}">
				<input type="hidden" name="type" value="MLT_rl_insert">
...
sub _display_comp_team_table {
  ...
  my $st = qq[
    ...
    FROM
      tblComp_Teams
      LEFT JOIN tblTeam USING(intTeamID)
      LEFT JOIN tblClub USING(intClubID)
    WHERE
  ...
  while (my $href = $q->fetchrow_hashref()) {
		...
		my $comp_status_link = '';
		if ($status eq 'Active') {
			$comp_status_link = qq[[<a href="index.cgi?$Data->{'client'}&type=MLT_cs_inactive&id=$href->{'intCompNO'}" onclick="return confirm('Are you sure you wish to set the Status in Competition to Inactive ?')">Make Inactive</a>]];
		}
		else {
			$comp_status_link = qq[[<a href="index.cgi?$Data->{'client'}&type=MLT_cs_active&id=$href->{'intCompNO'}" onclick="return confirm('Are you sure you wish to set the Status in Competition to Active ?')">Make Active</a>]];
		}
		my $team_status_link = '';
    if ($team_status eq 'Active') {
      $team_status_link = qq[[<a href="index.cgi?$Data->{'client'}&type=MLT_t_inactive&id=$href->{'intTeamID'}" onclick="return confirm('Are you sure you wish to set the Team Status to Inactive ?')">Make Inactive</a>]];
    }
    else {
      $team_status_link = qq[[<a href="index.cgi?$Data->{'client'}&type=MLT_t_active&id=$href->{'intTeamID'}" onclick="return confirm('Are you sure you wish to set the Team Status to Active ?')">Make Active</a>]];
    }
		my $team_delete_link = '';
    if ($team_status eq 'Active' or $team_status eq 'Inactive') {
      $team_delete_link = qq[[<a href="index.cgi?$Data->{'client'}&type=MLT_t_delete&id=$href->{'intTeamID'}" onclick="return confirm('Are you sure you wish to set the Team Status to Deleted ?')">Make Deleted</a>]];
    }
    my $clear_club_link = '';
    if ($href->{'intClubID'} and $href->{'intClubID'} > 0) {
      $clear_club_link = qq[<a href="index.cgi?$Data->{'client'}&type=MLT_clear_team_club&intClubID=$href->{'intClubID'}&intTeamID=$href->{'intTeamID'}">[Clear Club]</a>];
    }		
		if ($href->{intTeamID} == -1) {
			$href->{'strName'} = 'Bye';
			$comp_status_link = '';
    ...
    $comp_teams_html .= qq[
			<tr>
				...
				<td>$href->{'strClubName'} $clear_club_link</td>
			</tr>
		];
  }
  ...
  $comp_teams_html = qq[
    <table class="listTable">
      <tr><th>Team ID</th><th>Team</th><th>Status in Competition</th><th>Team Status</th><th>Club</th></tr>
      $comp_teams_html
    </table>
  ] if $comp_teams_html;
...
sub _display_teams_in_fixture {
  my ($Data) = @_;
	...
	foreach my $key (keys %{$teams}) {
    next if $key eq 'earliest_fixture_date';
		$teams->{$key}{'Name'} = 'Bye' if $key == -1;
		$teams_in_fixture_html .= qq[<tr><td>$key</td><td>$teams->{$key}{'Name'}</td><td>$teams->{$key}{'Rounds'}</td></tr>];
	}
  $teams_in_fixture_html = qq[<tr><td colspan="4">No records found</td>] unless($teams_in_fixture_html);
  $teams_in_fixture_html = qq[
    <table class="listTable">
      <tr><th>Team ID</th><th>Team</th><th>Round Numbers</th></tr>
...
sub _reset_comp_teams {
  ...
  my $q_select_teams = $Data->{'db'}->prepare($st_select_teams);
	...
	while (my $href = $q_select_teams->fetchrow_hashref()) {
		$existing_teams{$href->{'intTeamID'}} = 1;
	}
...
sub _reset_match_numbers_in_fixture {
  ...
  my $st = qq[
    SELECT
			CM.intMatchID,
      CM.intRecStatus as CMintRecStatus,
      CR.intRecStatus as CRintRecStatus,
      CM.intHomeTeamID,
      CM.intAwayTeamID,
      CM.dtMatchTime,
			CM.intMatchNum,
	...
	FROM
     ...
     LEFT JOIN tblCompRounds CR ON CR.intRoundID = CM.intRoundID

    WHERE
      CM.intCompID = ?
      order by CM.intPoolID,
	CR.intRoundNumber ASC,
	CM.intMatchNum ASC
	;
  ];
  my $q = $Data->{'db'}->prepare($st);
  ...
  $q->execute($Data->{'clientValues'}{'compID'});
  while (my $href = $q->fetchrow_hashref()) {
		$i=1 if($href->{'intPoolID'}!=$poolID);
		$intRecStatus =0;
		if($href->{'CRintRecStatus'}==-1 or $href->{'CRintRecStatus'}==-1) {
		$intRecStatus = -1;
		}
		$q_update->execute($i, $intRecStatus, $href->{'intMatchID'});
		$i++ if($href->{'CMintRecStatus'}>-1 and $href->{'CRintRecStatus'}>-1);
		$poolID = $href->{'intPoolID'};
  }

  return;
}

sub _clear_team_club {
  my ($Data) = @_;
  my $team_id_IN = param('intTeamID') || 0;
  my $club_id_IN = param('intClubID') || 0;
  unless ($team_id_IN and $club_id_IN) {
    return;
  }
  my $st = qq[
    UPDATE
  ...
  my $q = $Data->{'db'}->prepare($st);
  $q->execute($team_id_IN, $club_id_IN);
  return;
}
...
#      LEFT JOIN tblCompRounds AS R ON (CM.intRoundID = R.intRoundID)
#		WHERE
#			CM.intCompID = ?
...
#		if (not defined $teams{'earliest_fixture_date'} or  $href->{'dtMatchTime'} lt $teams{'earliest_fixture_date'}) {
...
sub _get_teams_in_fixture {
  ...
  my %teams = ();
  while (my $href = $q->fetchrow_hashref()) {
    $teams{$href->{'intHomeTeamID'}}{'Name'} = $href->{'strHomeTeam'};
    $teams{$href->{'intHomeTeamID'}}{'Rounds'} .= $href->{'intRoundNumber'} . "&nbsp;";
    $teams{$href->{'intAwayTeamID'}}{'Name'} = $href->{'strAwayTeam'};
    $teams{$href->{'intAwayTeamID'}}{'Rounds'} .= $href->{'intRoundNumber'} . "&nbsp;";
    next if not defined $href->{'dtMatchTime'};
    if (not defined $teams{'earliest_fixture_date'} or  $href->{'dtMatchTime'} lt $teams{'earliest_fixture_date'}) {
      $teams{'earliest_fixture_date'} = $href->{'dtMatchTime'};
    }
  }
