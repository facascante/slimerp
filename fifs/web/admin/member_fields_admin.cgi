...
#
# $Header: svn://svn/SWM/trunk/web/techadmin/member_fields_admin.cgi 8249 2013-04-08 08:14:07Z rlee $
#
...
sub get_member_field_layout {
  ...
  my ($configchanges, $realmID, $stID)  = get_field_layout_blob($db, $scID);
   $body = qq[
    ...
    <form name="form" action="member_fields_admin.cgi" method="post">
...
sub generate_groups_ddl {
  my ($field, $group_IN) = @_;
  my @groups = qw(interests details contact identification profile contracts citizenship parent financial medical custom1 other);
  my $ddl = '';
  for my $group (@groups) {
