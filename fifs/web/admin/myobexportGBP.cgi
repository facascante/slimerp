...
#
# $Header: svn://svn/SWM/trunk/web/admin/myobexportGBP.cgi 10074 2013-12-01 22:58:24Z tcourt $
#
...
<br>
            <label class="label": for "txtGBPRate">GBP Rate:</label><input type="text" name="gbprate" id="txtGBPRate" value="0.6">
...
sub buildDoc {
    ...
    my $GBPRate= param("gbprate")         || 0;

    return getErrorMessage() if !$GBPRate;
  ...
  #  $doc .= "\r\n";
	...
	my %ItemAmount=();
	my $currentKey='';
	my $exchangeRate = $GBPRate; #0.6; ## CHANGE FOR GBP
	my $paypalTotal=0;
    while (my $dref = $query->fetchrow_hashref()) {
        my $currency    = $dref->{strCurrencyCode} || 'AUD';
	my $job = $dref->{strMYOBJobCode} || '';
	next if $currency ne 'GBP';
	my $country = 'United Kingdom';
	next if ! $job;
	### If gbpis used, then need to look at GST Rate and Exchange column
...
$doc.= "\n";
    ...
    my $body = qq[
        ...
        <form action ="$target" method="POST">
            ...
            <input type="hidden" name="gbprate" value="$GBPRate">
...
sub doExport {
    ...
    my $message  = "The GBP data you requested for export is included in the attached file ($filename)" ;
    my $subject  = "GBP Data Export - Payment Split Fees";
    ...
    if ($retval == 0) {
...

        {
            ...
            my $subject  = "GBP Data Export - Customer List";
        ...
        }
	{
            ...
            my $message  = "The customer list for export ID $myobExportID is included in the attached file ($filename)" ;
            my $subject  = "GBP Data Export - PayPal Fees";
