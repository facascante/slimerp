#
# $Header: svn://svn/SWM/trunk/web/finder/Finder.pm 8251 2013-04-08 09:00:53Z rlee $
#
...
sub search_results {
  ...
  if ($postcode_IN =~ /^\d{4}$/ or $search_IN) {
    ...
    $extra_WHERE .= qq[AND A.intAssocID IN ($clubLevelAssoc)] if ($clubLevelAssoc and $clubLevel);
		my $statement = qq[
			SELECT 
				...
				S.intShowTreasurer, 
				S.intShowRegistrar,
        S.strEmail AS strVenueEmail,
        S.strFax AS strVenuePhone,
				SR.strSubTypeName, 
				A.intAssocTypeID
			FROM tblAssoc AS A 
    ...
    $SearchResults{'clubLevelAssoc'} = $clubLevelAssoc;
    $result = runTemplate(
      $Data,
      \%SearchResults,
