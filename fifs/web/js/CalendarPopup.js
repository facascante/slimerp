...
// Butchered Slightly by Warren Rodie warren@sportingpulse.com   31/10/01
// To allow passing of update field through code

// Butchered again by Warren Rodie 7/1/03 to allow passing of form name
...
cal.showCalendar(anchorname, formname);
...
function CalendarPopup_showCalendar(anchorname, formname) {
	this.aname=anchorname;
...
function CalendarPopup_getCalendar() {
	...
	for (var row=1; row<=6; row++) {
		...
		for (var col=1; col<=7; col++) {
			...
			result += '	<TD CLASS="'+td_class+'"><A HREF="javascript:'+windowref+this.returnFunction+'('+display_year+','+display_month+','+display_date+',\''+anchorname+'\',\''+formname+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');" CLASS="'+date_class+'">'+display_date+'</A></TD>\n';
			display_date++;
	...
	result += '		<A CLASS="todaylink" HREF="javascript:'+windowref+this.returnFunction+'(\''+now.getFullYear()+'\',\''+(now.getMonth()+1)+'\',\''+now.getDate()+'\',\''+anchorname+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');">Today</A>\n';
	result += '		<BR>\n';
